...
import pickle
import os, cv2
from preprocessing_thread import parse_annotation, BatchGenerator,threadsafe_iter
...
yadk_yolo = load_model('basic-yolo-keras/yad2k/model_data/yolo.h5')

layers = model.layers

for layer in layers:
    if ('conv' in layer.name and layer.name not in LAYER_SCRATCH):
        yadk_name = layer.name.replace("conv", "conv2d") 
        weights = yadk_yolo.get_layer(yadk_name).get_weights() 
        
        layer.set_weights(weights)
        
        print ('Weights from layer {} used in layer {}, layer has {} weights'.format(yadk_name,layer.name,weights[0].shape))

del layers, yadk_yolo       
'''

model = make_parallel(model, gpu_count=2)
model.load_weights('weights/yolo_fish.h5')
...
train_batch = BatchGenerator(train_imgs, generator_config, jitter=True, aug_freq = AUG_FREQ)
valid_batch = BatchGenerator(valid_imgs, generator_config, jitter=True, aug_freq = AUG_FREQ)

steps_per_epoch  = train_batch.get_dateset_size()
validation_steps = valid_batch.get_dateset_size()

train_batch = train_batch.get_generator()
valid_batch = valid_batch.get_generator()

train_batch = threadsafe_iter(train_batch)
valid_batch = threadsafe_iter(valid_batch)
...
checkpoint = ModelCheckpoint('weights/yolo_fish_2.h5', 
                             monitor='val_loss', 
                             verbose=1, 
                             save_best_only=True, 
                             mode='min', 
...
tb_counter  = len([entry for entry in os.listdir('logs/') if 'yolo_' in entry]) + 1

tensorboard = TensorBoard(log_dir='logs/yolo_' + str(tb_counter), 
                          histogram_freq=0, 
                          write_graph=True, 
                          write_images=False)

csv_logger = CSVLogger('logs/yolo_fish_2.csv', separator=',', append=False)
