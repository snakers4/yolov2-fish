...
import pickle
import os, cv2
from preprocessing_normalize import parse_annotation, BatchGenerator
...
model.load_weights('weights/yolo_fish_class_norm.h5')
...
train_batch = BatchGenerator(train_imgs, generator_config, jitter=AUG, aug_freq = AUG_FREQ, shuffle=True)
valid_batch = BatchGenerator(valid_imgs, generator_config, jitter=AUG, aug_freq = AUG_FREQ, shuffle=True)
...
checkpoint = ModelCheckpoint('weights/yolo_fish_class_norm_ft.h5', 
                             monitor='val_loss', 
                             verbose=1, 
                             save_best_only=True, 
                             mode='min', 
...
csv_logger = CSVLogger('logs/yolo_fish_class_norm_ft.csv', separator=',', append=False)
